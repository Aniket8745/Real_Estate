...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://real-estate-58jo.onrender.com/api/user/update/${currentUser._id}`, {
        method: 'POST',
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(`https://real-estate-58jo.onrender.com/api/user/delete/${currentUser._id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success === false) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch('https://real-estate-58jo.onrender.com/api/auth/signout');
      const data = await res.json();
      if (data.success === false) {
        dispatch(deleteUserFailure(data.message));
        return;
  ...
  const handleShowListings = async () => {
    try {
      ...
      const res = await fetch(`https://real-estate-58jo.onrender.com/api/user/listings/${currentUser._id}`, {
      headers: {
        Authorization: `Bearer ${currentUser.token}`,
      },
    });
  ...
  const handleListingDelete = async (listingId) => {
    try {
      const res = await fetch(`https://real-estate-58jo.onrender.com/api/listing/delete/${listingId}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success === false) {
